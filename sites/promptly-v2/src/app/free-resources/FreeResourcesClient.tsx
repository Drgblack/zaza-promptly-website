...
const content = {
  en: {
    title: 'Free Resources for Teachers',
    subtitle: 'Download free templates, guides, and tools to enhance your teaching practice and save time.',
    searchLabel: 'Search Resources',
    searchPlaceholder: 'Search by title or description...',
    filterLabel: 'Filter by format:',
    resultsText: (count: number) => `${count} resource${count !== 1 ? 's' : ''} found`,
    noResultsTitle: 'No resources found',
    noResultsText: 'Try adjusting your search or filter criteria.',
    licenseLabel: 'Usage License',
    openButton: 'Open',
    downloadButton: 'Download',
    ctaTitle: 'Need More Support?',
    ctaText: 'Explore our Learning Centre for expert insights and visit our blog for the latest tips.',
    learningCentreButton: 'Visit Learning Centre',
    blogButton: 'Read Blog'
  },
  de: {
    title: 'Kostenlose Ressourcen für Lehrer',
    subtitle: 'Laden Sie kostenlose Vorlagen, Leitfäden und Tools herunter, um Ihre Unterrichtspraxis zu verbessern und Zeit zu sparen.',
    searchLabel: 'Ressourcen suchen',
    searchPlaceholder: 'Nach Titel oder Beschreibung suchen...',
    filterLabel: 'Nach Format filtern:',
    resultsText: (count: number) => `${count} Ressource${count !== 1 ? 'n' : ''} gefunden`,
    noResultsTitle: 'Keine Ressourcen gefunden',
    noResultsText: 'Versuchen Sie, Ihre Such- oder Filterkriterien anzupassen.',
    licenseLabel: 'Nutzungslizenz',
    openButton: 'Öffnen',
    downloadButton: 'Herunterladen',
    ctaTitle: 'Benötigen Sie mehr Unterstützung?',
    ctaText: 'Erkunden Sie unser Lernzentrum für Experteneinblicke und besuchen Sie unseren Blog für die neuesten Tipps.',
    learningCentreButton: 'Lernzentrum besuchen',
    blogButton: 'Blog lesen'
  }
}
...
export default function FreeResourcesClient() {
  ...
  const formatDate = (dateString: string) => {
    return new Date(dateString).toLocaleDateString(currentLanguage === 'de' ? 'de-DE' : 'en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    })
        ...
        <div className="container">
          <ScrollReveal duration={0.24} delay={0.1}>
            <div className="max-w-4xl mx-auto mb-8">
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                <span className="text-sm font-medium text-white mr-4">{copy.filterLabel}</span>
                {formats.map((format) => (
                  <button
                    key={format}
          ...
          <div className="max-w-4xl mx-auto mb-6">
            <p className="text-slate-400 text-sm">
              {copy.resultsText(filteredResources.length)}
            </p>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            {filteredResources.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="bg-slate-800/50 border border-slate-600/50 rounded-lg p-3 mb-4">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <p className="text-xs font-medium text-amber-400 mb-1">{copy.licenseLabel}</p>
                    ...
                    <div className="flex gap-3">
                      {resource.format === 'PDF' ? (
                        <a
                          ...
                          aria-label={`${copy.openButton} ${resource.title} (${resource.format}) in new tab`}
                      ...
                      <a
                        ...
                        aria-label={`${copy.downloadButton} ${resource.title} (${resource.format})`}
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-white mb-2">{copy.noResultsTitle}</h3>
                <p className="text-slate-400">{copy.noResultsText}</p>
              </div>
            )}
          </div>
        </div>
